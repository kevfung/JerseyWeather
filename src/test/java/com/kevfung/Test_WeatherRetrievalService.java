...
public class Test_WeatherRetrievalService extends JerseyTest {	
	...
	private WeatherApiUtil mockWeatherApiUtil;
	
	@Override
    protected Application configure() {
		mockWeatherApiUtil = mock(OpenWeatherApiUtil.class);
		
        return new ResourceConfig(WeatherRetrievalService.class)
        		.property(WeatherServiceApplication.WEATHER_UTIL_PROPERTY, mockWeatherApiUtil);
    }	
	...
	public void test_getCurrentWeather_success() {
		...
		when(mockWeatherApiUtil.getCurrentWeather()).thenReturn(OPEN_WEATHER_API_JSON_RESPONSE);
	...
	public void test_getCurrentWeather_failure_JSON_null() {
		...
		PowerMockito.mockStatic(JacksonUtil.class);
		PowerMockito.when(JacksonUtil.jsonToObj(OPEN_WEATHER_API_JSON_RESPONSE, CurrentWeather.class)).thenReturn(null);
